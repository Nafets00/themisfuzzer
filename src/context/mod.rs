...
pub struct PBFTContext {
    pub pbft: PBFT,
    pub r: Receiver<PBFTMessage>,
    pub c: Receiver<Message<Raw<Client>>>,
    pub a: Receiver<app::Command>,
}
...
pub fn add_pbft_config(config: &mut Config) {
    ...
    let pbftc = config_pbft::PBFTConfig::default();
    config.set("pbft", pbftc).expect("set pbft");
}

pub fn setup_pbft() -> PBFTContext {
...
pub fn setup_with_checkpoint(primary: bool, checkpoint_interval: u64) -> PBFTContext {
    let mut config = config::default();
    add_pbft_config(&mut config);
    config
